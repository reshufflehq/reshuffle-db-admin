  ...
  const handleGet =  async (_, res) => {
    try {
      ...
      for (var x = 0; x < result.length; x++) {
        output += `<tr><td>${result[x].key}</td><td><textarea id="${
          result[x].key
        }" rows="4" cols="50">${JSON.stringify(result[x].value)}</textarea>
        </td><td>
        <button class="button delete" onclick="deleteElement('${
          result[x].key
        }')">Delete</button>
        <button class="button update" onclick="updateElement('${
          result[x].key
        }')">Update</button>
        </td></tr>`;
      }
